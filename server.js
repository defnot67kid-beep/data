...
function initRoles() {
    ...
    if (!data.roles || data.roles.length === 0) {
        ...
        data.nextRoleId = DEFAULT_ROLES.length + 1;
        writeData(data);
    }
}
...
app.put('/api/profile', authenticateToken, (req, res) => {
    try {
        ...
        // Update fields
        if (displayName !== undefined && displayName !== null) users[index].displayName = displayName;
        ...
        // Ensure owner has correct badge (ALWAYS)
        ...
        // Ensure moderator has correct badge (ALWAYS)
...
app.delete('/api/user/:username', authenticateToken, (req, res) => {
    let users = readUsers();
    if (req.user.username !== req.params.username) return res.status(403).json({ error: 'Forbidden' });
...
app.post('/api/migrate-ids', (req, res) => {
    ...
    data.users.forEach(user => {
        ...
        if (user.roleBadge === undefined) user.roleBadge = "";
        if (user.roleColor === undefined) user.roleColor = "#888888";
...
app.listen(PORT, () => {
    console.log(`\n========================================`);
    console.log(`🟣 Tavian Backend Server (FULLY FIXED)`);
    console.log(`========================================`);
    console.log(`📡 Running on: http://localhost:${PORT}`);
    console.log(`🔗 Frontend URL: ${FRONTEND_URL}`);
    console.log(`========================================`);
    console.log(`✨ Fixed Features:`);
    console.log(`   • Profile editing now works! ✅`);
    console.log(`   • Owner badge shows for realgysj 👑`);
    console.log(`   • Moderator badge shows for plstealme2 🔨`);
